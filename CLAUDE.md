...
- **実装ファイル（`*.kt` / `*.cpp` / `build.gradle.kts` 等）に Read・Edit・Write ツールでアクセスする前に、以下を必ず確認する（実装前チェックリスト）**
  1. `tasks/task-list.md` に対応する 🔄 進行中タスクが存在するか？
  2. そのタスクの Specify（仕様・受入条件）がユーザーに確認済みか？
  - どちらかが NO なら、実装ツールを使わず、まず Specify 作成 → タスク登録 → ユーザー確認を行う

- **実装・ドキュメント変更・ルール更新を問わず、必ず「検討結果の報告 → ユーザー確認 → 実作業」の順を守る**
  - 分析・提案の依頼を受けた場合も同様。検討結果のみを先に報告し、承認を得てからファイル操作に入る
  - **`CLAUDE.md` / `memory/` / `DESIGN.md` 等のルール・方針文書への変更も、コードと同じルールを適用する。「提案 → ユーザー承認 → 反映」の例外なし**
